...
import 'package:wherenxnew1/ApiCallingPage/EditEventWithImage.dart';
import 'package:wherenxnew1/UserScerrn/EventDetails.dart';

import '../ApiCallingPage/EditEventWithOutImage.dart';
...
class _EditEventDetailsState extends State<EditEventDetails> {
  ...
  String filePath1 = "", videofilepath = "",
      userName = "", eventName = "", eventType = "", eventDate = "", eventAddress = "",eventDesc = "", endeventDate= "",
      retriveimage = "1";
  ...
  SharedPreferences? preferences;
   ...
   Future<void> retrievePayment() async {
    ...
    eventId = preferences.getInt("eventId") ?? 0;
    eventName = preferences.getString("eventName") ?? "";
    eventType = preferences.getString("eventType") ?? "";
    eventDate = preferences.getString("eventDate") ?? "";
    endeventDate = preferences.getString("endeventDate") ?? "";
    eventAddress = preferences.getString("eventAddress") ?? "";
    eventDesc = preferences.getString("eventDesc") ?? "";
    filepathdet = preferences.getStringList("evevtimagelist") ?? [];
    ...
    if(filepathdet.isNotEmpty){
     ...
     print("userDetaildataqmm   ${filepathdet}");

    textYouName.text = userName;
    textYourEventName.text = eventName;
    textEventType.text = eventType;
    textEventAddress.text = eventAddress;
    textEventDescription.text = eventDesc;
    currentDateTime.text = eventDate;
    endingDateTime.text = endeventDate;
                              ...
                              child: Container(
                                ...
                                child: ClipRRect(
                                  ...
                                  child: SizedBox.fromSize(
                                    ...
                                    // Image radius
                                    child: retriveimage == "2" ? Image.file(File(filepathdet[index]), fit: BoxFit.cover)
                                        : Image.network(filepathdet[index],fit: BoxFit.cover),
                                  ),
                                ),
                              ),
                            );
                  ...
                  Container(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        if (textYouName.text.toString().isEmpty) {
                        ...
                        } else if (textEventDescription.text
                            .toString()
                            .isEmpty) {
                          Left_indicator_bar_Flushbar(
                          ...
                          if(filepathdet[0].contains("http://")){

                            ViewDialog(context: context).showLoadingIndicator(
                              "Edit Event Wait...", "Event Details", context);

                          SharedPreferences pre =
                              await SharedPreferences.getInstance();
                          int userId = pre.getInt("userId") ?? 0;
                          String str_userId = userId.toString();
                          String str_eventId = eventId.toString();

                          String videofilepath1 = "";
                          String str_image = filepathdet[0];


                          http.StreamedResponse? response = await EditEventWithOutImage()
                              .editEventDetails(
                                  str_eventId,
                                  str_userId,
                          ...
                          print(
                              "usereventdetails ${str_eventId} ${str_userId}  ${textYouName.text.toString()}  ${textYourEventName.text.toString()}  "
                              "${textEventType.text.toString()} "
                              "${currentDateTime.text.toString()}  ${endingDateTime.text.toString()} ${textEventAddress.text.toString()} "
                              "${textEventDescription.text.toString()}"
                         ...
                         // String str_image = filepathdet[0];
                          ...
                          http.StreamedResponse? response = await EditEventWithImage()
                              .editEventDetails(
                                  str_eventId,
                                  str_userId,
                                  textYouName.text.toString(),
                           ...
                           http.StreamedResponse? response1 = await EditEventWithImage()
                              .editEventImageDetails(str_eventId,filepathdet);

                          print("usereventdetails ${str_eventId}  ${filepathdet}");
                      ...
                      child: const Text('Edit Your Event Now',
